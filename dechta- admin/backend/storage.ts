...
let locationIdColumnPromise: Promise<"user_id" | "entity_id"> | null = null;
let manpowerPricingCleanupDone = false;

async function getLocationIdColumn(): Promise<"user_id" | "entity_id"> {
  if (!locationIdColumnPromise) {
    locationIdColumnPromise = pool
      ...
      .then((result) => {
        ...
        if (columns.has("user_id")) return "user_id" as const;
        if (columns.has("entity_id")) return "entity_id" as const;
...
function toLocationIdValue(column: "user_id" | "entity_id", entityId: string) {
  if (column === "user_id") {
...
async function ensureAdminOnboardingCompatibilitySchema(): Promise<void> {
  ...
  if (await tableExistsInPublic('manpower_pricing')) {
    await pool.query(`ALTER TABLE manpower_pricing ADD COLUMN IF NOT EXISTS service_category TEXT`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE manpower_pricing ADD COLUMN IF NOT EXISTS service_code TEXT`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE manpower_pricing ADD COLUMN IF NOT EXISTS base_price NUMERIC`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE manpower_pricing ADD COLUMN IF NOT EXISTS rate_per_hour NUMERIC`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE manpower_pricing ADD COLUMN IF NOT EXISTS min_hours NUMERIC DEFAULT 1`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE manpower_pricing ADD COLUMN IF NOT EXISTS estimated_duration TEXT`).catch(() => ({ rows: [] as any[] }));
...
export interface IStorage {
  ...
  deleteAllManpowerPricing(): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async upsertLocation(data: InsertLocationUpdate): Promise<LocationUpdate> {
    ...
    await pool.query(
      `DELETE FROM location_updates WHERE entity_type = $1 AND ${idColumn} = $2`,
      [data.entityType, idValue]
    );
  ...
  async updateManpowerPricing(id: number, data: Partial<InsertManpowerPricing & { isActive?: boolean }>): Promise<ManpowerPricing> {
    const map: Record<string, string> = {
    ...
    vals.push(id);
    const r = await pool.query(
      `UPDATE manpower_pricing SET ${sets.join(', ')} WHERE id = $${vals.length}
       RETURNING id,
         service_category   AS "serviceCategory",
         service_name       AS "serviceName",
         service_code       AS "serviceCode",
  ...
  async deleteAllManpowerPricing(): Promise<void> {
